...
const Meetcalendar = (props) => {
    ...
    useEffect(() => {
        ...
        const idx = meetState.datetimes.findIndex(dt => dt.date === formattedFocusedDate);

        const foundTimes = meetState.datetimes[idx]?.times || [] 

        dispatch(setMeet({ ...meetState, currentDate: formattedFocusedDate, currentTimes: foundTimes }));
    }, [focusedDate])
    ...
    const onCalendarChange = (array) => {
        ...
        if (valueToAdd) {
            ...
            const rmvIxd = meetState.datetimes.findIndex(e => e.date === valueToRemove);
